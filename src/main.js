...
import { initGyroscope, setupKeyboardInput } from "./input.js";
...
function handlePauseResume() {
  if (state.gameState === "paused") transitionTo("resuming");
}
function handlePauseRestart() {
  if (state.gameState === "paused") transitionTo("countdown");
}
function handlePauseLeaderboard() {
  if (state.gameState === "paused") transitionTo("pausedLeaderboard");
}
...
dom.pauseResumeBtn.addEventListener("touchend", (e) => {
  e.preventDefault();
  handlePauseResume();
});
dom.pauseRestartBtn.addEventListener("touchend", (e) => {
  e.preventDefault();
  handlePauseRestart();
});
dom.pauseLbBtn.addEventListener("touchend", (e) => {
  e.preventDefault();
  handlePauseLeaderboard();
});
dom.pauseTrackBtn.addEventListener("touchend", (e) => {
  e.preventDefault();
  handlePauseTrackSelect();
});
...
document
  .getElementById("lb-tab-global")
  .addEventListener("click", () => setLeaderboardTab("global"));
document
  .getElementById("lb-tab-local")
  .addEventListener("click", () => setLeaderboardTab("local"));
document.getElementById("lb-tab-global").addEventListener("touchend", (e) => {
  e.preventDefault();
  setLeaderboardTab("global");
});
document.getElementById("lb-tab-local").addEventListener("touchend", (e) => {
  e.preventDefault();
  ...
  if (
    ...
    if (e.code === "ArrowLeft" || e.code === "KeyA") {
      ...
      state.selectedTrackIdx =
        (state.selectedTrackIdx - 1 + TRACKS.length) % TRACKS.length;
      buildTrackSelectUI();
    ...
    if (!state.nameEntryState.done) {
      ...
      state.highlightDistance = state.finishDistance;
    ...
    state.gameState === "pausedLeaderboard" &&
    (e.code === "Space" || e.code === "Enter")
  ) {
    e.preventDefault();
    transitionTo("paused");
