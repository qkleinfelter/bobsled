...
import { NUM_SECTORS, PHYSICS, TRACK_VISUAL } from "./constants.js";
import { dom } from "./dom.js";
import { state } from "./state.js";
import { findPointIndex, getTrackPointAt, TRACKS } from "./tracks.js";
...
function drawTrackSurface(startIdx, endIdx, scale) {
  ...
  for (let i = startIdx; i <= endIdx && i < pts.length; i += 8) {
    ...
    ctx.beginPath();
    ctx.moveTo(
      (p.x + Math.cos(perp) * -hw) * scale,
      (p.y + Math.sin(perp) * -hw) * scale,
    );
    ctx.lineTo(
      (p.x + Math.cos(perp) * hw) * scale,
      (p.y + Math.sin(perp) * hw) * scale,
...
function drawOptimalLine(startIdx, endIdx, scale) {
  ...
  for (let i = startIdx; i <= endIdx && i < pts.length; i++) {
    const p = pts[i];
    const optD =
      -Math.sign(p.curvature) *
      hw *
      0.6 *
      Math.min(1, Math.abs(p.curvature) * 20);
    const perp = p.angle + Math.PI / 2;
...
function drawDistanceMarkers(startIdx, endIdx, scale) {
  ...
  for (let i = startIdx; i <= endIdx && i < pts.length; i++) {
    ...
    if (m % 100 === 0 && Math.abs(p.dist - m) < 0.3) {
      ...
      ctx.beginPath();
      ctx.moveTo(
        (p.x + Math.cos(perp) * (-hw + 0.3)) * scale,
        (p.y + Math.sin(perp) * (-hw + 0.3)) * scale,
      );
      ctx.lineTo(
        (p.x + Math.cos(perp) * (-hw + 0.8)) * scale,
        (p.y + Math.sin(perp) * (-hw + 0.8)) * scale,
      );
      ctx.stroke();
    }
  }
...
function drawStartFinish(startIdx, endIdx, scale) {
  ...
  if (startIdx === 0) {
    ...
    ctx.beginPath();
    ctx.moveTo(
      (sp.x + Math.cos(sPerp) * -hw) * scale,
      (sp.y + Math.sin(sPerp) * -hw) * scale,
    );
    ctx.lineTo(
      (sp.x + Math.cos(sPerp) * hw) * scale,
      (sp.y + Math.sin(sPerp) * hw) * scale,
  ...
  if (endIdx >= track.points.length - 2) {
    ...
    ctx.beginPath();
    ctx.moveTo(
      (fp.x + Math.cos(fPerp) * -hw) * scale,
      (fp.y + Math.sin(fPerp) * -hw) * scale,
    );
    ctx.lineTo(
      (fp.x + Math.cos(fPerp) * hw) * scale,
      (fp.y + Math.sin(fPerp) * hw) * scale,
...
function renderHUD() {
  ...
  if (state.infiniteMode) {
    // Show distance in center instead of track info
    dom.hudTrack.textContent = formatDistance(state.sled.s);
    dom.hudTrack.style.color = "#ab47bc";
    dom.hudTrack.style.fontSize = "20px";
  } else {
