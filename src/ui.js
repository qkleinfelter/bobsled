...
import { resetSled, state } from "./state.js";
import { TRACKS, generateInfiniteTrack, generateTrack } from "./tracks.js";
...
export function transitionTo(newState) {
  ...
  switch (newState) {
    ...
    case "countdown":
      ...
      {
        const trackDef = TRACKS[state.selectedTrackIdx];
        state.infiniteMode = trackDef.id === "infinite";
        if (state.infiniteMode) {
          state.track = generateInfiniteTrack();
        } else {
          state.track = generateTrack(trackDef);
        }
      }
      dom.progressBar.style.display = state.infiniteMode ? "none" : "block";
      resetSled();
      break;
    ...
    case "finish":
      ...
      dom.progressBar.style.display = state.infiniteMode ? "none" : "block";
      if (state.infiniteMode) {
        document.querySelector("#finish-screen h2").textContent = "CRASHED!";
        document.querySelector("#finish-screen h2").style.color = "#ef5350";
        dom.finalTimeEl.textContent = formatDistance(state.finishDistance);
        dom.finalTopSpeedEl.textContent =
          "Time: " +
          formatTime(state.finishTime) +
          "  \u00B7  Top Speed: " +
          Math.round(state.topSpeed * 2.237) +
          " mph";
      } else {
        document.querySelector("#finish-screen h2").textContent = "FINISH!";
        document.querySelector("#finish-screen h2").style.color = "#4fc3f7";
        dom.finalTimeEl.textContent = formatTime(state.finishTime);
        dom.finalTopSpeedEl.textContent =
          "Top Speed: " + Math.round(state.topSpeed * 2.237) + " mph";
      }
      // Start name entry
    ...
    case "leaderboard":
      ...
      document.getElementById("leaderboard-prompt").textContent =
        "TAP OR PRESS SPACE TO SELECT TRACK";
      renderLeaderboard();
      break;
...
export function buildTrackSelectUI() {
  ...
  TRACKS.forEach((t, idx) => {
    ...
    card.className =
      "track-card" + (idx === state.selectedTrackIdx ? " selected" : "");
...
export function buildNameEntryUI() {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    slot.className =
      "letter-slot" + (i === state.nameEntryState.cursor ? " active" : "");
    slot.id = "letter-" + i;
...
export function updateNameEntryUI() {
  for (let i = 0; i < 3; i++) {
    ...
    slot.className =
      "letter-slot" + (i === state.nameEntryState.cursor ? " active" : "");
    slot.querySelector(".char").textContent =
      ALPHABET[state.nameEntryState.letters[i]];
  }
}
...
export function handleNameEntryInput(e) {
  ...
  if (
    state.nameKeyDebounce[e.code] &&
    now - state.nameKeyDebounce[e.code] < 150
  )
    return;
  ...
  if (e.code === "ArrowUp" || e.code === "KeyW") {
    state.nameEntryState.letters[cur] =
      (state.nameEntryState.letters[cur] - 1 + ALPHABET.length) %
      ALPHABET.length;
    updateNameEntryUI();
    ...
    if (cur < 2) {
      ...
      const name = state.nameEntryState.letters
        .map((i) => ALPHABET[i])
        .join("");
      saveToLeaderboard(
        name,
  ...
  for (let i = 0; i < 3; i++) {
    ...
    if (upArrow) {
      upArrow.addEventListener("touchend", function (e) {
        e.stopPropagation();
        state.nameEntryState.cursor = i;
        state.nameEntryState.letters[i] =
          (state.nameEntryState.letters[i] - 1 + ALPHABET.length) %
          ALPHABET.length;
        updateNameEntryUI();
      });
    }
